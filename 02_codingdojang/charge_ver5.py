...
string_free = '"축하합니다. 1주년 이벤트에 당첨되었습니다. 여기 무료 티켓을 발행합니다. 잔여무료티켓 [{0}]장"'
string_discount='"축하합니다.연간회원권 구매 이벤트에 당첨되셨습니다. 연간 회원 할인 티켓을 발행합니다. 잔여 할인티켓 [{0}]장"'
free_ticket=3
discount_ticket=5
count_free=0
count_discount=0
...
while True: 
    ...
    if charge_type==1 :
        ...
        if age_input >= 60 and age_input <= 65 :
            print(string_card_payment.format(int(charge*0.85*0.95)))
        else :
    ...
    if count_free%7==0 :
        if free_ticket>0 : 
            free_ticket-=1
            print(string_free.format(free_ticket))
    if count_discount%4==0 :
        if discount_ticket>0:
            discount_ticket-=1
            print(string_discount.format(discount_ticket))
